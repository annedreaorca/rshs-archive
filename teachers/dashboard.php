<?php $pageTitle = 'Dashboard';
    ...
    // $sql = "SELECT SUM(quantity) FROM `lab_equipments` WHERE is_archived = 0";
    // $result = $conn->prepare($sql);
    // $result->execute();
    // $total_books = $result->fetchColumn();

    // $sql = "SELECT count(*) FROM `lab_equipments` WHERE access_level = 2";
    // $result = $conn->prepare($sql);
    // $result->execute();
    // $total_students = $result->fetchColumn();

    // $sql = "SELECT count(*) FROM `lab-equipments` WHERE status = 'Borrowed'";
    // $result = $conn->prepare($sql);
    // $result->execute();
    // $total_borrowed_books = $result->fetchColumn();
?>
<section class="flex">
